...
# Load data
path = "dataset" # <-- CHANGE THE PATH NAME TO THE DATASET FOLDER
...
# Remove outliers
reduced_x_train = clean_outliers(reduced_x_train)
reduced_x_test = clean_outliers(reduced_x_test)
...
# Set the optimal parameters for the method
best_degree = 1
best_lambda = 0.5

# Build polynomial from the data (x becomes (1,x))
tx_tr = build_poly(standardized_x_train, 1)
tx_te = build_poly(standardized_x_test, 1)

# Run Regularized Logistic Regression with the training set
...
create_csv_submission(test_ids, y_pred, "best_result.csv")
